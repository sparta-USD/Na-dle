...
def music_grades_merge():
    grades = pd.read_csv('data/grades_data.csv')
    musics = pd.read_csv('data/music_data.csv')
...
def collaborative_filtering(title_user):
    ...
    csv_path = 'data/user_distance_data.csv'
...
def recommend_musics(user_id):
    ...
    user_based_collab = pd.read_csv('data/user_distance_data.csv')
    user_based_collab.index=music_grades.index
    user_based_collab.columns=music_grades.index

    # # # 1번 유저와 가장 비슷한 유저를 뽑고,
    user = user_based_collab[user_id].sort_values(ascending=False)[:10].index[1]
